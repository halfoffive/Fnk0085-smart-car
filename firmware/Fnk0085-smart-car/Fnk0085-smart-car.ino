 ...
 * 版本：v0.3.3（与 backend/Cargo.toml、frontend/package.json 同步）
 * -----------------------------------------------------------------
...
const uint16_t POLL_TASK_STACK   = 20480; // pollTask 栈字节
const uint16_t VIDEO_TASK_STACK  = 20480; // videoTask 栈字节（HTTPClient + TLS 握手栈占用大）
const uint16_t TELEMETRY_TASK_STACK = 20480; // telemetryTask 栈字节（独立客户端上报遥测）
...
const uint8_t  CMD_QUEUE_LEN     = 16;
...
// 原子地设置 TLS 握手失败标志（videoTask core 0 / pollTask core 0 跨核安全）
...
int httpsPost(const String& path, const String& body, String& respOut, uint32_t timeoutMs) {
  ...
  http.end();
  if (code < 0 || code >= 300) {
    if (useTls) { httpsClient.stop(); } else { plainClient.stop(); }
  }
  httpsLockGive();
  return code;
}
...
int httpsGet(const String& path, String& respOut) {
  ...
  http.end();
  if (code != 200) {
    if (useTls) { pollSecureClient.stop(); } else { pollClient.stop(); }
  }
  return code;
}
 ...
 * 使用 HTTP/1.1 keep-alive 长连接复用，减少每帧重连开销
...
int httpsPostFrame(uint8_t* jpeg, size_t len, uint64_t uptimeMs) {
  ...
  // 成功路径：保持 keep-alive 长连接复用，减少下帧重连延迟
...
void telemetryTask(void* arg) {
  ...
  for (;;) {
    ...
    http.end();
    if (code != 200) {
      if (useTls) { telemetrySecureClient.stop(); } else { telemetryClient.stop(); }
      Serial.printf("[NET] telemetry failed code=%d\n", code);
    }
  }
}
...
/* =================================================================
 * 视频采集任务（FreeRTOS，core 0，与 WiFi 协议栈同核，避免跨核 WiFi API 调用死锁）
 ...
 * 关键：拷贝即还，esp_camera_fb_get() 和 memcpy 到 PSRAM 在 photoMux 保护下快速执行，
 *       网络 POST 也在 core 0 与 WiFi 协议栈协同，更安全，避免跨核 WiFi API 调用死锁
 * ================================================================= */
void videoTask(void* arg) {
  ...
  for (;;) {
    ...
    if (!fb) {
      Serial.printf("[VIDEO] fb_get failed, skip frame\n");
    ...
    if (total > 0 && total % 10 == 0) {
      Serial.printf("[VIDEO] frames ok=%u fail=%u stack=%u freeHeap=%u freePSRAM=%u\n",
...
void setup() {
  ...
  // 先初始化所有 SecureClient（无论 useHttps，TLS 回退时也可能用到）
  ...
  // 初始化明文客户端（无论 useHttps，TLS 回退时也需要）
  plainClient.setTimeout(POLL_HTTP_TIMEOUT_MS);
  pollClient.setTimeout(POLL_HTTP_TIMEOUT_MS);
  telemetryClient.setTimeout(2000);
  videoClient.setTimeout(FRAME_POST_TIMEOUT_MS);

  // 根据 useHttps 打印对应模式的客户端就绪日志
  if (useHttps) {
    ...
    Serial.println("[HTTP] poll client ready");
    Serial.println("[HTTP] telemetry client ready");
    Serial.println("[HTTP] video client ready");
  }
  ...
  // 启动视频采集任务（core 0，与 WiFi 协议栈同核，避免跨核 WiFi API 调用死锁）
  xTaskCreatePinnedToCore(videoTask, "video", VIDEO_TASK_STACK, NULL, 1, &videoTaskHandle, 0);

  // 启动 HTTPS 长轮询任务（core 0，与 WiFi 协议栈同核，避免跨核 WiFi API 调用）
