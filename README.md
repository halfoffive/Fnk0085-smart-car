...
```
┌─────────────┐  UDP+AEAD(8分包,视频)  ┌──────────────┐  HTTPS+SSE  ┌─────────────┐
│  ESP32-S3   │ ────────────────────▶│  Rust 后端   │ ───────────▶│  Web PWA    │
│ 摄像头/电机 │                       │ actix-http   │             │ Vite+Tailwind│
│ 编码器/PID  │ ◀────HTTPS 长轮询──── │ 多设备转发    │ ◀───────────│ Web Worker  │
│             │   register/poll/event │ per-device   │   控制指令    │             │
└─────────────┘                       └──────────────┘             └─────────────┘
```

控制平面与视频平面分离：
- **视频平面**：UDP 7000 + AES-128-GCM AEAD（10fps、≤100ms 延时，等价 DTLS 安全属性）
- **控制平面**：HTTPS 8080 + Bearer token，设备长轮询拉指令、POST 上报事件；自签证书由 `WiFiClientSecure::setInsecure()` 信任（生产应换 mTLS / CA pinning）

- **固件** ([firmware/](firmware/))：Arduino `.ino`，QVGA 10fps、L298N 双电机、双槽型光电编码器、PID 平衡 + PWM 缓存、UXGA 拍照存 SD（SD_MMC setPins 39/38/40）、Web Serial 配网 + token 串口打印、AES-128-GCM AEAD（等价 DTLS）、HTTPS 控制通道（`WiFiClientSecure` + FreeRTOS `pollTask`）
- **后端** ([backend/](backend/))：Rust + actix-http v3.13.1（启 http2 feature），多设备 DashMap、每设备 1s 视频环形缓存 + per-device 指令队列、gzip、token+SSL（mTLS）、`include_dir` 内嵌前端单二进制部署
- **前端** ([frontend/](frontend/))：Vite 8.1.0 + Vue 3.5.9 + TailwindCSS 4.3.1，工业控制台美学、WASD + 1-100 调速滑块、Web Worker 取流 + 延时显示、拍照加载圈、Web Serial 配网（含 host:port 校验）、PWA injectManifest 离线缓存
- **协议** ([protocol.md](protocol.md))：UDP 视频分包字节布局 + HTTPS 设备 API + 浏览器 API + Web Serial 帧格式
...
2. 安装依赖：`ESP32 Arduino` 核心 3.x、`ArduinoJson`；SD_MMC / WiFiClientSecure / HTTPClient 已随核心自带
3. 选板子 `ESP32S3 Dev Module`，启用 PSRAM，烧录
4. 烧录后串口（115200）会进入配网等待状态，发送 `CONFIG\n` 可查询已存配置
5. 通过前端 Web Serial 弹窗一键下发 `CONFIG|ssid=...|password=...|server=<host:8080>|token=...` 至 NVS；设备收到后自动重启并连接
...
- **后端**：Rust stable、actix-http 3.13.1（http2 + rustls-0_23 + compress-gzip）、rustls 0.23、DashMap、tokio broadcast、include_dir、build.rs 自动构建前端
