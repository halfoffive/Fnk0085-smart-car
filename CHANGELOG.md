...
- 新增 GitHub Actions 自动构建后端（`.github/workflows/build-backend.yml`）：覆盖 7 个目标平台（Linux gnu/musl × x86_64/aarch64、Windows x86_64、macOS x86_64/aarch64），构建产物打包后上传至 Amazon S3；触发条件为 push master（版本=`latest`）、push tag `v*`（版本=tag 名）、手动触发
