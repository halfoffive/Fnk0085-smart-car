...
- **固件日志帧率极低（~0.3-1fps）且 WASD 命令延时高（最坏 ~30s）**：根因有两个。① 视频通道每帧 `videoSecureClient.stop()` 导致全 TLS 握手（1-3s/帧），且未启用 `setSession()` 会话复用。② `pollTask` 30s 长轮询持 `httpsMutex`，`loop()` 的 `sendTelemetry` 等锁阻塞 `dispatchCommands()`，WASD 指令排在遥测 POST 之后。已修复：视频通道启用 `WiFiClientSecure::session_t` 会话复用 + 成功路径不 `stop()` 保持连接；`pollTask` 改用独立 `pollSecureClient`/`pollClient`，不再持 `httpsMutex`；`loop()` 中 `dispatchCommands()` 移到 `sendTelemetry()` 之前。
- **HTTP + WiFi 环境下命令延时仍有 ~0.5s**：根因是 `sendTelemetry()` 仍在 `loop()`（core 1）中同步执行 HTTP POST，WiFi 往返 100-300ms 期间 `loop()` 被阻塞，`dispatchCommands()` 在 POST 期间到达的 WASD 指令要等下一个 loop 周期才能消费。已修复：新增 `telemetryTask`（core 0，独立 `telemetrySecureClient`/`telemetryClient` + `setSession()` 会话复用），以 100ms 周期读取 RPM 并 POST 上报；`loop()` 中移除 `sendTelemetry()` 调用，仅保留 `dispatchCommands()` + `encoderSample()` + PID + `motionStopAt` 等本地操作，loop 周期 < 1ms，命令延时降到 <10ms。
